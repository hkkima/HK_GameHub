...
## 가장 쉬운 방법: 허브에서 바로 올리기

허브에 **이름·PIN 으로 로그인**한 뒤 오른쪽 위 **＋ 게임 올리기** 를 누르세요.

- **`index.html` 파일 하나짜리 게임**(Claude 가 만들어 준 단일 HTML)이면, 끌어다 놓고
  제목만 적어 **게시**하면 **바로 갤러리에 뜹니다.** 검토도 빌드도 없습니다.
- 이미지·스크립트가 **여러 파일로 나뉜 게임**(React 등)은 **폴더째 선택**하면 됩니다.
  올리면 자동으로 정리·빌드되어 **몇 분 뒤** 갤러리에 올라옵니다. `node_modules` 는
  빼고 올려 주세요(합계 8MB 이하). `package.json` 이 있으면 빌드형으로 처리됩니다.
  React/Vite 앱은 `vite.config.js` 에 `base: './'` 를 넣어 주세요.

> 단일 HTML 게임은 격리된 샌드박스에서 실행됩니다. 그래서 **`localStorage` 같은 저장
> 기능은 동작하지 않습니다**(최고점수 저장 등). 세션 안에서 도는 게임은 문제없습니다.
> 저장이 꼭 필요하면 아래 폴더 방식으로 올리세요.

아래는 GitHub 로 직접 올리고 싶을 때의 방법입니다.
...
|---|---|---|
| `title` | ✅ | 게임 이름 |
| `author` | ✅ | 제작자 이름 (화면에 표시) |
| `authorId` | ⚠️ | **포인트를 받을 학급 계정.** 수업에서 쓰는 이름을 소문자로, 공백은 `_` 로 바꿔 적습니다. 빠지면 좋아요 포인트를 못 받습니다 |
| `cohort` | | 기수 |
...
받은 좋아요 1건당 **500 포인트**를 받습니다. 강사가 매주 승인하면 학급 계정
잔액으로 들어갑니다.

- 포인트는 `manifest.json` 의 **`authorId`** 로 지급됩니다. 이 값이 없거나 학급
  계정과 다르면 지급 대상에서 빠지니 꼭 확인하세요
- 좋아요는 **1인 1게임 1회**입니다. 로그인해야 누를 수 있습니다
- 이미 지급된 좋아요는 취소할 수 없습니다
