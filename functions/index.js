...
// 업로드 파일들을 GitHub blob 용으로 검증·정리한다. dataUrl(base64) 에서 순수
// base64 만 뽑는다. 진입점(index.html / package.json) 유무를 함께 알려 준다.
function cleanUploadFiles(files) {
  let total = 0;
  const cleaned = [];
  let hasEntry = false;
  ...
  return { cleaned, hasEntry };
}

// manifest.json 은 폼 값으로 우리가 만든다(수강생이 넣은 건 무시).
function buildManifest({ title, authorName, authorId, meta, spa }) {
  return {
    title,
    author: authorName,
    authorId,
...
// apps/<slug>/manifest.json 을 읽는다. 없으면 null. 소유 확인·삭제 대상 파악에 쓴다.
async function readManifest(token, base, slug) {
  try {
    const r = await gh(token, 'GET', `${base}/contents/apps/${slug}/manifest.json?ref=${REPO_BRANCH}`);
...
// apps/<slug>/ 아래 모든 파일 경로를 훑는다(재귀 트리). 교체·삭제 시 옛 파일 정리에 쓴다.
async function listAppFiles(token, base, treeSha, slug) {
  const t = await gh(token, 'GET', `${base}/git/trees/${treeSha}?recursive=1`);
...
// tree 항목 배열을 커밋 → 브랜치 → PR → 자동 머지한다. 신규·수정·삭제가 공유한다.
async function commitTreeAndMerge(token, base, { baseSha, baseTreeSha, tree, message, prBody, branchSlug }) {
  const newTree = await gh(token, 'POST', `${base}/git/trees`, { base_tree: baseTreeSha, tree });
  const commit = await gh(token, 'POST', `${base}/git/commits`, { message, tree: newTree.sha, parents: [baseSha] });
  const branch = `submit/${branchSlug}`;
  await gh(token, 'POST', `${base}/git/refs`, { ref: `refs/heads/${branch}`, sha: commit.sha });
  const pr = await gh(token, 'POST', `${base}/pulls`, { title: message, head: branch, base: REPO_BRANCH, body: prBody });
...
export const gamehubSubmit = onCall(
  ...
  async (req) => {
    ...
    const op = String(req.data?.op || 'create');
    ...
    // 신뢰 모델: auth 는 익명이라 서버가 누구인지 모른다. authorId 는 클라이언트가 보낸
    // 값을 쓰되, 실재하는 참가자인지만 확인한다. 수정·삭제는 여기에 더해 대상 게임의
    // 기존 manifest.authorId 와 일치해야 한다(남의 게임을 건드리지 못하게).
    const author = await db.collection('users').doc(authorId).get();
    if (!author.exists) throw new HttpsError('not-found', '등록되지 않은 참가자입니다.');
    const authorName = author.data().name || authorId;
    ...
    // 브랜치 충돌을 피할 짧은 접미사. 함수 런타임에서 crypto 를 쓸 수 있다.
    const nonce = (globalThis.crypto?.randomUUID?.() || `${Date.now()}`).replace(/\D/g, '').slice(0, 6);

    // ── 수정·삭제: 기존 게임 소유 확인 ──────────────────────────────────
    if (op === 'update' || op === 'delete') {
      const slug = String(req.data?.slug || '').trim();
      if (!SLUG_RE.test(slug)) throw new HttpsError('invalid-argument', '잘못된 게임 식별자입니다.');

      const existing = await readManifest(token, base, slug);
      if (!existing) throw new HttpsError('not-found', '대상 게임을 찾을 수 없습니다.');
      if (String(existing.authorId || '').trim() !== authorId) {
        throw new HttpsError('permission-denied', '이 게임을 수정·삭제할 권한이 없습니다.');
      }

      if (op === 'delete') {
        const paths = await listAppFiles(token, base, baseCommit.tree.sha, slug);
        if (!paths.length) throw new HttpsError('not-found', '삭제할 파일이 없습니다.');
        const tree = paths.map((path) => ({ path, mode: '100644', type: 'blob', sha: null }));
        const res = await commitTreeAndMerge(token, base, {
          baseSha, baseTreeSha: baseCommit.tree.sha, tree,
          message: `game: delete ${existing.title || slug} (${authorName})`,
          prBody: `허브에서 삭제 요청으로 자동 생성된 PR 입니다.\n\n- 제작자: ${authorName} (\`${authorId}\`)\n- 대상: \`apps/${slug}/\`${FOOTER}`,
          branchSlug: `del-${slug}-${nonce}`,
        });
        return { ...res, slug, deleted: true };
      }

      // op === 'update'
      if (!title) throw new HttpsError('invalid-argument', '제목이 필요합니다.');
      const tree = [];
      let spa = existing.spa === true;

      if (files.length) {
        if (files.length > MAX_FILES) throw new HttpsError('invalid-argument', `파일이 너무 많습니다(${files.length} > ${MAX_FILES}).`);
        const { cleaned, hasEntry } = cleanUploadFiles(files);
        if (!hasEntry) throw new HttpsError('invalid-argument', 'index.html 또는 package.json 이 없습니다.');

        // 파일 통째 교체. 새 목록에 없는 옛 파일만 지운다(겹치는 경로는 덮어쓰기로 처리).
        const newPaths = new Set(cleaned.map(({ rel }) => `apps/${slug}/${rel}`));
        newPaths.add(`apps/${slug}/manifest.json`);
        const old = await listAppFiles(token, base, baseCommit.tree.sha, slug);
        for (const path of old) if (!newPaths.has(path)) tree.push({ path, mode: '100644', type: 'blob', sha: null });
      ...
      // 파일을 안 줬으면 base_tree 가 기존 파일을 그대로 유지하고, manifest 만 갱신된다.

      const manifest = buildManifest({ title, authorName, authorId, meta, spa });
      ...
      const res = await commitTreeAndMerge(token, base, {
        baseSha, baseTreeSha: baseCommit.tree.sha, tree,
        message: `game: update ${title} (${authorName})`,
        prBody: `허브 수정으로 자동 생성된 PR 입니다.\n\n- 제작자: ${authorName} (\`${authorId}\`)\n- 대상: \`apps/${slug}/\`\n- ${files.length ? `파일 교체: ${files.length}개` : '메타데이터만 수정'}${FOOTER}`,
        branchSlug: `upd-${slug}-${nonce}`,
      });
      return { ...res, slug, updated: true };
    }
    ...
    const spa = files.some((f) => /(^|\/)(vite\.config|next\.config)\./.test(f.path || ''));
    const manifest = buildManifest({ title, authorName, authorId, meta, spa });
    ...
    const res = await commitTreeAndMerge(token, base, {
      baseSha, baseTreeSha: baseCommit.tree.sha, tree,
      message: `game: ${title} (${authorName})`,
      prBody: `허브 업로드로 자동 생성된 PR 입니다.\n\n- 제작자: ${authorName} (\`${authorId}\`)\n- 파일: ${cleaned.length}개${FOOTER}`,
      branchSlug: slug,
    });
    return { ...res, slug };
  },
