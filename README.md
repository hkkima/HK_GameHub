...
이름과 PIN 은 기존 학급 앱과 같지만, 검증을 **서버에서** 합니다.

```
이름·PIN 입력 → gamehubLogin 함수가 pinHash 대조
             → uid = 참가자 ID 인 커스텀 토큰 발급
             → signInWithCustomToken()
```

기존 앱들은 클라이언트에서 `pinHash` 를 대조하고 익명 로그인을 씁니다. 그 방식은
`request.auth.uid` 가 익명 uid 라 보안 규칙이 본인 확인을 할 수 없어서, **남의 이름으로
좋아요를 눌러 포인트를 만들어 낼 수 있습니다.** 좋아요 1건이 500 포인트로 환산되는
이상 그건 인플레 구멍이라, 허브만 커스텀 토큰 방식을 씁니다.

이름 슬러그와 PIN 해시는 기존 앱 구현과 결과가 완전히 같아야 합니다
([`hub/auth.js`](hub/auth.js), [`functions/index.js`](functions/index.js)).
다르면 같은 이름·PIN 으로 로그인이 안 됩니다.

Google 로그인은 운영자 전용이며, 주간 포인트 승인 화면에만 씁니다.
...
- 잔액 증가는 전부 Cloud Functions(Admin SDK)가 하므로, 클라이언트가 포인트를
  만들어 낼 수 있는 경로는 없습니다
