...
기존 학급 앱과 동일합니다. 이름·PIN 을 클라이언트에서 대조하고 Firebase 는 익명
로그인만 씁니다. Google 로그인은 운영자 전용이며 주간 포인트 승인 화면에만 씁니다.

```
이름·PIN 입력 → users/{이름슬러그}.pinHash 대조 → signInAnonymously()
```

Firebase 계정은 익명이고 학급 신원은 Firestore 의 `users` 문서라, 좋아요도 익명
uid 가 아니라 **참가자 ID** 로 기록됩니다. 그래서 기기를 바꿔도 좋아요가 따라옵니다.

이름 슬러그와 PIN 해시는 기존 앱 구현과 결과가 완전히 같아야 합니다
([`hub/auth.js`](hub/auth.js)). 다르면 같은 이름·PIN 으로 로그인이 안 됩니다.

> **트레이드오프**: 익명 uid 는 학급 신원이 아니라, 보안 규칙이 "이 사람이 정말
> 이 참가자인가" 를 확인하지 못합니다. 즉 마음먹으면 콘솔에서 남의 이름으로 좋아요를
> 만들 수 있습니다. 카운터 검증(참가자당 1문서, 실제 문서가 있어야 +1)은 그대로라
> 우연히 부풀지는 않고 카운트 상한도 참가자 수지만, 원천 차단은 아닙니다.
> 소규모 신뢰 환경을 전제로 한 선택입니다. 원천 차단이 필요하면 PIN 을 서버에서
> 검증해 커스텀 토큰을 발급하는 함수를 붙이면 됩니다(이 경우 함수 런타임
> 서비스 계정에 `Service Account Token Creator` 역할이 필요합니다).

### 좋아요 포인트

좋아요 1건 = 500 포인트. 강사가 주간 단위로 승인하면 게임 제작자
(`manifest.json` 의 `authorId`)에게 지급됩니다.

- **멱등성** — 좋아요 문서에 지급 회차(`paidIn`)를 표시해 두므로 승인을 두 번 눌러도
  중복 지급되지 않습니다
- **되돌리기 방지** — 지급된 좋아요는 규칙이 삭제를 막습니다
- **감사 기록** — 기존 `ledger` 컬렉션에 `type: 'gamehub_like'` 로 남습니다
- 잔액 증가는 `gamehubPayout` 함수(Admin SDK)만 합니다. 클라이언트는 좋아요만
  기록하고 포인트는 건드리지 못합니다

> 위 인증 트레이드오프 때문에, 조작된 좋아요(콘솔로 만든 남의 이름 좋아요)도
> 지급 대상에 들어갈 수 있습니다. 강사가 승인 화면에서 "누가 누구에게" 를 보고
> 걸러낼 수 있도록, 지급은 자동이 아니라 **주간 수동 승인**으로 둡니다.
