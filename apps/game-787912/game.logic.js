...
class Component extends DCLogic {
  ...
  tick(){
    const s = this.state;
    if(s.report || s.promo) return;   // 정산·임용장 동안 근무 시간은 흐르지 않는다

    const auto = this.rates().auto;
    const patch = {
      stamps: s.stamps + auto,
      sh: Object.assign({}, s.sh, {stamps: s.sh.stamps + auto}),
      combo: s.combo > 0 ? Math.max(0, s.combo - 0.4) : 0,
      queue: s.queue + 1
    };
  ...
  pop(text, accent){
    ...
    setTimeout(()=>this.setState(s=>({floaters:s.floaters.filter(f=>f.id!==id)})), 900);
  }
  ...
  process(kind, force){
    ...
    setTimeout(()=>this.setState({docAnim:0,processing:false}), 520);
    setTimeout(()=>{
